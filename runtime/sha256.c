/* Adapted 2025 by Daniel Neo López Martínez from Andrew Appel repo
...
value coq_sha256_wrapper(struct thread_info *$tinfo, value input_list) {
	/* 1. Convert list to array and get length */
    size_t len = 0;
    value curr = input_list;

    /* Count elements */
    while (get_Coq_Init_Datatypes_list_tag(curr) != NIL) {  /* Not nil */
        len++;
        curr = get_args(curr)[1];  /* tail */
    }
	...
	value *args;
	value elem;
	value byte_to_uint = generate_byte_to_uint_fun($tinfo);
    for (size_t i = 0; i < len; i++) {
		args = get_args(curr);
        input_buf[i] = (unsigned char) Long_val(call($tinfo, byte_to_uint, args[0]));  /* head */
        curr = args[1];  /* tail */
    }

    /* 2. Call verified SHA-256 */
    unsigned char output_buf[32];  /* SHA-256 always outputs 32 bytes */
    SHA256(input_buf, len, output_buf);
    free(input_buf);

    /* 3. Convert result back to Coq list */
    value result = make_Coq_Init_Datatypes_list_nil();
	value uint_to_byte = generate_uint_to_byte_fun($tinfo);
    /* Build list in reverse */
    for (int i = 31; i >= 0; i--) {
		value byte = call($tinfo, uint_to_byte, Val_int(output_buf[i]));
        result = alloc_make_Coq_Init_Datatypes_list_cons($tinfo, byte, result);
    }
